...
final class ProductoDAO extends DAO
{
    ...
    protected function crearEntidad(array $fila): object {
        $p = new Producto($fila['nombre'], (float)$fila['precio'], (int)$fila['stock'], (string)$fila['descripcion']);
    ...
    public function guardar(object $entidad): bool {
        ...
        $sql = "UPDATE {$this->tabla} SET nombre=:n, precio=:p, stock=:d, descripcion=:d WHERE id=:id";
        $st  = $this->pdo->prepare($sql);
        return $st->execute([
            ':n'=>$entidad->nombre,
            ':p'=>$entidad->precio,
